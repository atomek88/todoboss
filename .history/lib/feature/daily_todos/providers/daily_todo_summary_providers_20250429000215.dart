...
final allDailyTodoSummariesProvider = FutureProvider<List<DailyTodo>>((ref) async {
...
final dailyTodoByDateProvider = FutureProvider.family<DailyTodo?, DateTime>((ref, date) async {
  final repository = ref.watch(todoDateRepositoryProvider);
  try {
...
final dailyTodoPerformanceProvider = FutureProvider<Map<DateTime, Map<String, dynamic>>>((ref) async {
...
final filteredDailyTodoPerformanceProvider = FutureProvider<Map<DateTime, Map<String, dynamic>>>((ref) async {
  final allMetrics = await ref.watch(dailyTodoPerformanceProvider.future);
  
  // Filter out days with no activity
  return Map.fromEntries(
    allMetrics.entries.where((entry) {
...
class DailyTodoSummaryServiceNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  DailyTodoSummaryServiceNotifier(this._ref) : super(const AsyncValue.data(null));
  ...
  Future<void> generateTodaySummary() async {
    ...
    try {
      final repository = _ref.read(todoDateRepositoryProvider);
...
final dailyTodoSummaryServiceProvider = StateNotifierProvider<DailyTodoSummaryServiceNotifier, AsyncValue<void>>(
