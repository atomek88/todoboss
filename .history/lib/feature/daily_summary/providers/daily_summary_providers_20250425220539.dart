...
final allDailySummariesProvider = FutureProvider<List<DailySummary>>((ref) async {
...
final dailySummariesInRangeProvider = FutureProvider.family<List<DailySummary>, ({DateTime startDate, DateTime endDate})>(
  (ref, params) async {
    final repository = ref.watch(dailySummaryRepositoryProvider);
...
final lastNWeeksSummariesProvider = FutureProvider.family<List<DailySummary>, ({int weeks})>(
  (ref, params) async {
    final repository = ref.watch(dailySummaryRepositoryProvider);
...
final dailySummaryByDateProvider = FutureProvider.family<DailySummary?, DateTime>(
  (ref, date) async {
    final repository = ref.watch(dailySummaryRepositoryProvider);
...
class DailySummaryServiceNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  Future<DailySummary> generateTodaySummary() async {
    ...
    return repository.generateDailySummaryForDate(
      targetDate, 
      completedIds, 
      deletedIds,
      createdIds,
