...
class DailyTodoRepository {
  ...
  Future<DailyTodo?> getDailyTodoById(String id) async {
    try {
      ...
      if (dailyTodoIsar != null) {
        // Load associated todos by their IDs
        final todoIds = dailyTodoIsar.todos;
        List<Todo> todos = [];
  ...
  Future<List<DailyTodo>> getAllDailyTodos() async {
    try {
      ...
      // Get all todos once to avoid multiple database calls
      final allTodoIsars =
          await _isar!.collection<TodoIsar>().where().findAll();
      final allTodos = allTodoIsars.map((t) => t.toDomain()).toList();

      // Map of todo ID to Todo object for quick lookup
      ...
      final dailyTodos = dailyTodoIsars.map((dailyTodoIsar) {
        // Get todos for this dailyTodo
        final todos = dailyTodoIsar.todos
            .map((id) => todoMap[id])
  ...
  Future<bool> saveDailyTodo(DailyTodo dailyTodo) async {
    try {
      ...
      debugPrint(
          '📅 [DailyTodoRepository] Saved DailyTodo: ${dailyTodo.id} with ${dailyTodo.todos.length} todos');
      return true;
    } catch (e) {
