...
class TodoRepository {
  ...
  Future<List<Todo>> getTodos() async {
    try {
      ...
      talker.success('[TodoRepository] ===== TODOS LOADED =====');
      talker.info('[TodoRepository] Total todos: ${todos.length}');
      todos.forEach((todo) {
        talker.debug('[TodoRepository] Todo: ${todo.id} - ${todo.title} - Priority: ${todo.priority} - Status: ${todo.status} - Rollover: ${todo.rollover}');
        if (todo.hasSubtasks) {
          talker.debug('[TodoRepository]   Has ${todo.subtasks!.length} subtasks');
        }
        if (todo.isScheduled) {
          talker.debug('[TodoRepository]   Scheduled: ${todo.scheduled}');
        }
      });
      talker.success('[TodoRepository] ========================');
