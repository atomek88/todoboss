...
class TodoDateRepository {
  ...
  Future<TodoDate?> getTodoDateById(String id) async {
    try {
      ...
      final todoDateIsars = await _isar!
          .collection<TodoDateIsar>()
          .where()
          .findAll();
          
      final todoDateIsar = todoDateIsars.where((td) => td.dateId == id).firstOrNull;
          
      if (todoDateIsar != null) {
        debugPrint('📅 [TodoDateRepository] Found TodoDate: ${todoDateIsar.dateId}');
  ...
  Future<TodoDate> getTodoDateForDate(DateTime date, {int defaultGoal = 0}) async {
    try {
      await initialized;
      ...
      // Format ID from date
      final dateId = '${normalizedDate.day.toString().padLeft(2, '0')}${normalizedDate.month.toString().padLeft(2, '0')}${normalizedDate.year}';
      ...
      if (todoDate == null) {
        ...
        debugPrint('📅 [TodoDateRepository] Created new TodoDate for: $normalizedDate with goal: $defaultGoal');
      } else {
        ...
        if (defaultGoal > 0 && todoDate.taskGoal != defaultGoal) {
          ...
          await saveTodoDate(todoDate);
          debugPrint('📅 [TodoDateRepository] Updated TodoDate with default goal: $defaultGoal (was: ${todoDate.taskGoal})');
        } else {
          debugPrint('📅 [TodoDateRepository] Using existing TodoDate with goal: ${todoDate.taskGoal}');
        }
      }
  ...
  Future<List<TodoDate>> getAllTodoDates() async {
    try {
      ...
      final todoDateIsars = await _isar!.collection<TodoDateIsar>().where().findAll();
      
      final todoDates = todoDateIsars.map((todoDateIsar) => todoDateIsar.toDomain()).toList();
  ...
  Future<bool> saveTodoDate(TodoDate todoDate) async {
    try {
      ...
      final existing = todoDateIsars.where((td) => td.dateId == todoDate.id).firstOrNull;
  ...
  Future<bool> deleteTodoDate(String id) async {
    try {
      ...
      final todoDateIsars = await _isar!.collection<TodoDateIsar>().where().build().findAll();  
      final existing = todoDateIsars.where((td) => td.dateId == id).firstOrNull;
          
      if (existing == null) {
        debugPrint('📅 [TodoDateRepository] TodoDate not found for deletion: $id');
        return false;
      }
  ...
  Future<TodoDate?> addTodoToDate(DateTime date, String todoId) async {
    try {
      ...
      if (!todoDate.todoIds.contains(todoId)) {
        ...
        debugPrint('📅 [TodoDateRepository] Added todo $todoId to date ${todoDate.id}');
        return updatedTodoDate;
      } else {
        debugPrint('📅 [TodoDateRepository] Todo $todoId already on date ${todoDate.id}');
        return todoDate;
      }
    } catch (e) {
  ...
  Future<TodoDate?> removeTodoFromDate(DateTime date, String todoId) async {
    try {
      ...
      if (todoDate.todoIds.contains(todoId)) {
        ...
        debugPrint('📅 [TodoDateRepository] Removed todo $todoId from date ${todoDate.id}');
        return updatedTodoDate;
      } else {
        debugPrint('📅 [TodoDateRepository] Todo $todoId not on date ${todoDate.id}');
        return todoDate;
      }
    } catch (e) {
  ...
  Future<TodoDate?> updateTodoDateCounters(DateTime date, List<Todo> todos, {int defaultGoal = 0}) async {
    try {
      await initialized;
      ...
      debugPrint('📅 [TodoDateRepository] Calculating counters for date: ${normalizedDate.toString().split(' ')[0]}');
      ...
      final todosForDate = todos.where((todo) {
        ...
        final createdOnThisDate = DateTime(
            todo.createdAt.year, todo.createdAt.month, todo.createdAt.day) == normalizedDate;
            
        // For completed/deleted todos, check if they were ended on this date
        final endedOnThisDate = todo.endedOn != null && 
            DateTime(todo.endedOn!.year, todo.endedOn!.month, todo.endedOn!.day) == normalizedDate;
            
        final result = createdOnThisDate || endedOnThisDate;
        
        if (result) {
          debugPrint('📅 [TodoDateRepository] Including todo: ${todo.title} (status: ${todo.status})');
        }
      ...
      final completedCount = todosForDate.where((todo) => todo.status == 1).length;
      final deletedCount = todosForDate.where((todo) => todo.status == 2).length;
      ...
      debugPrint('📅 [TodoDateRepository] Found ${todosForDate.length} todos for date: ${normalizedDate.toString().split(' ')[0]}');
      debugPrint('📅 [TodoDateRepository] Completed: $completedCount, Deleted: $deletedCount, Goal: $taskGoal');
      ...
      final updatedTodoDate = todoDate.copyWith(
        ...
        summary: todoDate.generateSummary(todosForDate),
      );
      
      // Save it
      await saveTodoDate(updatedTodoDate);
      
      debugPrint('📅 [TodoDateRepository] Updated TodoDate counters for ${todoDate.id}: completed=$completedCount, deleted=$deletedCount, goal=$taskGoal');
      return updatedTodoDate;
    } catch (e) {
      debugPrint('📅 [TodoDateRepository] Error updating TodoDate counters: $e');
      return null;
    }
  }
  ...
  Future<TodoDate?> setTaskGoal(DateTime date, int goal) async {
    try {
      ...
      debugPrint('📅 [TodoDateRepository] Set task goal for ${todoDate.id} to $goal');
      return updatedTodoDate;
    } catch (e) {
