...
import 'package:todoApp/feature/todos/models/daily_todos_isar.dart';
...
class DailyTodoRepository {
  ...
  Future<DailyTodo?> addTodoToDate(DateTime date, String todoId) async {
    try {
      await initialized;
      ...
      final todoDate = await getDailyTodoForDate(normalizedDate);
      ...
      if (!todoDate.todoIds.contains(todoId)) {
        ...
        final updatedDailyTodo = todoDate.addTodoId(todoId);

        // Save the updated DailyTodo
        await saveDailyTodo(updatedDailyTodo);

        debugPrint(
            '📅 [DailyTodoRepository] Added todo $todoId to date ${todoDate.id}');
        return updatedDailyTodo;
      } else {
        debugPrint(
            '📅 [DailyTodoRepository] Todo $todoId already on date ${todoDate.id}');
        return todoDate;
      }
    } catch (e) {
  ...
  Future<DailyTodo?> removeTodoFromDate(DateTime date, String todoId) async {
    try {
      await initialized;
      ...
      if (todoDate.todoIds.contains(todoId)) {
        ...
        final updatedDailyTodo = todoDate.removeTodoId(todoId);

        // Save the updated DailyTodo
        await saveDailyTodo(updatedDailyTodo);

        debugPrint(
            '📅 [DailyTodoRepository] Removed todo $todoId from date ${todoDate.id}');
        return updatedDailyTodo;
      } else {
        debugPrint(
            '📅 [DailyTodoRepository] Todo $todoId not on date ${todoDate.id}');
        return todoDate;
      }
    } catch (e) {
  ...
  Future<DailyTodo?> updateDailyTodoCounters(DateTime date, List<Todo> todos,
    ...
    try {
      ...
      final todoDate = await getDailyTodoForDate(date, defaultGoal: goalToUse);
      ...
      final updatedDailyTodo = todoDate.copyWith(
      ...
      debugPrint(
          '📅 [DailyTodoRepository] Updated DailyTodo counters for ${todoDate.id}: completed=$completedCount, deleted=$deletedCount, goal=$taskGoal');
      return updatedDailyTodo;
    } catch (e) {
      debugPrint(
