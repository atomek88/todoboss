...
class _TodoSubtaskSectionState extends ConsumerState<TodoSubtaskSection> {
  ...
  void _addSubtask() {
    ...
    ref.read(todoListProvider.notifier).updateTodo(widget.parentTodo.id, updatedTodo);
  ...
  Widget build(BuildContext context) {
    ...
    final latestParentTodo = ref.watch(todoListProvider)
        .firstWhere((todo) => todo.id == widget.parentTodo.id, 
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (!_editControllers.containsKey(subtask.id)) {
                  _editControllers[subtask.id] = TextEditingController(text: subtask.title);
                }
                              ...
                              onTap: () {
                                ...
                                setState(() {
                                  ...
                                  if (!_editingSubtasks[subtask.id]!) {
                                    _editControllers[subtask.id]!.text = subtask.title;
                                  }
                                });
                              },
                              child: Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 3.0), // Reduced vertical padding
                                child: isEditing
                                  ? TextField(
                                      controller: _editControllers[subtask.id],
                                      decoration: const InputDecoration(
                                        isDense: true,
                                        contentPadding: EdgeInsets.symmetric(vertical: 6.0), // Reduced from 8.0
