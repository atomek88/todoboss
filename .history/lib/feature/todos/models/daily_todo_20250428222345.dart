...
class DailyTodo {
  ...
  final List<Todo> todos; // IDs of todos for this date
  ...
  DailyTodo addTodoId(Todo todo) {
    if (todos.contains(todo)) return this;
    return copyWith(todos: [...todos, todo]);
  }

  // Remove a todo ID from this date
  DailyTodo removeTodoId(Todo todo) {
    if (!todos.contains(todo)) return this;
    return copyWith(todos: todos.where((id) => id != todo).toList());
  }
