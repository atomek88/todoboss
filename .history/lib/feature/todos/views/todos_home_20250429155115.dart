...
class _TodosHomePageState extends ConsumerState<TodosHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    dailyTodoAsync.whenData((dailyTodo) {
      if (dailyTodo != null) {
        ...
        if (!dailyTodoDate.isAtSameMomentAs(normalizedSelectedDate)) {
          debugPrint(
              '⚠️ [TodosHomePage] Date mismatch detected: DailyTodo=${dailyTodo.date}, Selected=$normalizedSelectedDate');
          ...
          _dateStabilizationTimer =
            ...
            if (mounted) {
              debugPrint(
                  '🔄 [TodosHomePage] Synchronizing DailyTodo to selected date: $normalizedSelectedDate');
              ref
    ...
    if (isPastDate &&
      ...
      debugPrint(
          '🔄 [TodosHomePage] Selected date ($normalizedSelectedDate) is in the past, redirecting to PastDatePage');
    ...
    return dailyTodoAsync.when(
      data: (dailyTodo) {
        ...
        if (dailyTodo == null) {
          return _buildScaffold(context, todos, floatingActionButtons);
        }
  ...
  void _undoCompleteTodo() {
    if (_lastCompletedTodo != null && _lastCompletedId != null) {
      ...
      ref
          ...
          .updateTodo(_lastCompletedId!, _lastCompletedTodo!);
  ...
  Widget _buildScaffold(BuildContext context, List<Todo> todos,
