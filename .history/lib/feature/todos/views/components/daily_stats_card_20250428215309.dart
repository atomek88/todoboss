...
class DailyStatsCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final normalizedSelectedDate = normalizeDate(ref.watch(selectedDateProvider));
    
    // Watch the TodoDate provider to display stats
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    // Today button
                    _TodayButton(normalizedSelectedDate: normalizedSelectedDate),
...
class _TodayButton extends ConsumerWidget {
  ...
  const _TodayButton({required this.normalizedSelectedDate, Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return IconButton(
      ...
      onPressed: () {
        ...
        showDatePicker(
          context: context,
          initialDate: ref.read(currentDateProvider).subtract(const Duration(days: 1)),
          firstDate: ref.read(currentDateProvider).subtract(const Duration(days: 365)),
          lastDate: ref.read(currentDateProvider).subtract(const Duration(days: 1)),
        ).then((selectedDate) {
