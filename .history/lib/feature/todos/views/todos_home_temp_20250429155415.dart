...
class _TodosHomePageState extends ConsumerState<TodosHomePage> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      ...
      debugPrint('🚀 [TodosHomePage] Initialized with today\'s date: $todayDate');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    dailyTodoAsync.whenData((dailyTodo) {
      if (dailyTodo != null) {
        ...
        if (!dailyTodoDate.isAtSameMomentAs(normalizedSelectedDate)) {
          debugPrint('⚠️ [TodosHomePage] Date mismatch: DailyTodo=${dailyTodo.date}, Selected=$normalizedSelectedDate');
          
          // Debounce date synchronization
          _dateStabilizationTimer?.cancel();
          _dateStabilizationTimer = Timer(const Duration(milliseconds: 300), () {
            if (mounted) {
              debugPrint('🔄 [TodosHomePage] Syncing DailyTodo to selected date: $normalizedSelectedDate');
    ...
    if (isPastDate && !_isRedirecting && (_lastRedirectedDate == null || 
      ...
      debugPrint('🔄 [TodosHomePage] Date $normalizedSelectedDate is past, redirecting to PastDatePage');
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted && _isRedirecting) {
              Navigator.of(context).push(
                ...
                debugPrint('🔄 [TodosHomePage] Reset date to today: $todayDate');
              });
            }
          });
        }
      ...
      data: (dailyTodo) {
        ...
        final activeTodos = allTodos.where((todo) {
          if (isPastDate) {
            ...
            final targetDate = normalizeDate(dailyTodo?.date ?? normalizedSelectedDate);
