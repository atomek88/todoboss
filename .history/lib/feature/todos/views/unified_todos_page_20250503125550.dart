...
class UnifiedTodosPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final shouldShowPastView = isPastDate(selectedDate, includeYesterday: false);
    ...
    debugPrint('📅 [UnifiedTodosPage] Selected date: $formattedSelected (normalized: $normalizedSelectedDate)');
    debugPrint('📅 [UnifiedTodosPage] Current date: $formattedCurrent');
    debugPrint('📅 [UnifiedTodosPage] Showing ${shouldShowPastView ? "PAST" : "CURRENT"} view');
    
    return Scaffold(
      appBar: AppBar(
        title: Text(shouldShowPastView ? 'Past Date Summary' : 'Today\'s Tasks'),
        centerTitle: true,
