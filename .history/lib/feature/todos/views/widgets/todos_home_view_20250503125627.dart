...
class _TodosHomeViewState extends ConsumerState<TodosHomeView> {
  ...
  void _syncGoalWithDailyTodo() {
    ...
    if (!normalizedSelectedDate.isBefore(normalizedCurrentDate)) {
      ...
      dailyTodoNotifier.setGoal(goalValue);
    ...
    return dailyTodoAsync.when(
      data: (dailyTodo) {
        ...
        if (dailyTodo != null) {
          ...
          if (!normalizedDailyTodoDate.isAtSameMomentAs(normalizedSelectedDate)) {
            debugPrint('⚠️ [TodosHomeView] Date mismatch: DailyTodo=${dailyTodo.date}, Selected=$selectedDate');
            
            // Force a reload of the DailyTodo for the correct date
            WidgetsBinding.instance.addPostFrameCallback((_) {
              debugPrint('🔄 [TodosHomeView] Syncing DailyTodo to selected date: $selectedDate');
          ...
          debugPrint('📋 [TodosHomeView] Showing ${activeTodos.length} todos for date: ${dailyTodo.date} (day $selectedDayOfWeek)');
          debugPrint('📋 [TodosHomeView] Current DailyTodo has ${dailyTodo.todos.length} todos total');
        } else {
          debugPrint('⚠️ [TodosHomeView] No DailyTodo available for selected date');
        }
