...
class _TodosHomePageState extends ConsumerState<TodosHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    // Filter todos by status and date
    ...
    // Filter active todos by the selected date
    final activeTodos = todos.where((Todo todo) {
      // Check if todo was created on the selected date
      ...
      return todo.status == 0 && todoDate.isAtSameMomentAs(selectedDateTime);
    }).toList();
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const SwipeableDatePicker(
          suffixText: 'To-Do',
          dateTextStyle: TextStyle(fontWeight: FontWeight.bold),
          suffixTextStyle: TextStyle(fontWeight: FontWeight.w500),
        ),
        actions: [
          IconButton(
