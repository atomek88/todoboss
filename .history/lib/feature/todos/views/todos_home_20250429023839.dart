...
class _TodosHomePageState extends ConsumerState<TodosHomePage> {
  ...
  // Track if the DailyTodo is stable for this date
  bool _dailyTodoDateIsStable = false;
  ...
  // Update DailyTodo counters based on todos
  ...
  Widget build(BuildContext context) {
    ...
    // Watch the DailyTodo provider - this ensures we're using the right date state
    final dailyTodoAsync = ref.watch(dailyTodoProvider);

    // Check if the DailyTodo is stable for this date
      ...
      if (dailyTodo != null) {
        ...
        if (!dailyTodoDate.isAtSameMomentAs(normalizedSelectedDate)) {
          ...
          _dateStabilizationTimer =
            ...
            debugPrint(
                '🛠 [TodosHomePage] Applying date correction to DailyTodo: $normalizedSelectedDate');
            ref
                .read(dailyTodoProvider.notifier)
                .dateChanged(normalizedSelectedDate);
          });
        } else if (!_dailyTodoDateIsStable) {
          // Date matches and we need to mark as stable
          _dailyTodoDateIsStable = true;
          debugPrint(
              '✅ [TodosHomePage] DailyTodo stabilized for date: ${dailyTodo.date}');
        }
      }
    });
