...
class TodoDateFilterService {
  ...
  static List<Todo> getTodosForDate(List<Todo> allTodos, DateTime selectedDate) {
    ...
    return allTodos.where((todo) {
      ...
      final createdOnSelectedDate = todoCreationDate.isAtSameMomentAs(normalizedSelectedDate);
      ...
      final createdOnOrBeforeSelectedDate = !todoCreationDate.isAfter(normalizedSelectedDate);
      final shouldRollover = isRolloverAndActive && createdOnOrBeforeSelectedDate;
      
      // Condition 3: Todo is scheduled for this day of week using the new scheduledDays Set<int>
      final isScheduledForToday = todo.scheduledDays.contains(dayOfWeek);
      
      final shouldShow = createdOnSelectedDate || shouldRollover || 
                       (isRolloverAndActive && isScheduledForToday);
      
      if (shouldShow) {
        debugPrint('Including todo: ${todo.title} (created: ${todo.createdAt}, rollover: ${todo.rollover}, scheduled: ${todo.scheduled})');
      }
...
final filteredTodosProvider = Provider.family<List<Todo>, DateTime>((ref, selectedDate) {
