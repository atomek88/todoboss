...
class SelectedDateNotifier extends StateNotifier<DateTime> {
  ...
  void setDate(DateTime date) {
    ...
    debugPrint(
        '🗓 [SelectedDateNotifier] setDate called with: ${date.toString()}');
    debugPrint(
        '🗓 [SelectedDateNotifier] Normalized to: ${normalizedDate.toString()}');
    debugPrint('🗓 [SelectedDateNotifier] Current state: ${state.toString()}');

    // Only update if the date has actually changed
    if (normalizedDate != state) {
      debugPrint(
          '🗓 [SelectedDateNotifier] Date changing: ${state.toString()} → ${normalizedDate.toString()}');
      debugPrint(
          '🗓 [SelectedDateNotifier] Day of week changing: ${state.weekday} → ${normalizedDate.weekday}');
      state = normalizedDate;
    } else {
      debugPrint(
          '🗓 [SelectedDateNotifier] Ignoring duplicate date update: ${date.toString()}');
    }
  }
  ...
  void previousDay() {
    ...
    debugPrint(
        '🗓 [SelectedDateNotifier] Moving to previous day: ${state.toString()} → ${newDate.toString()}');
    state = newDate;
  }
  ...
  void nextDay() {
    ...
    debugPrint(
        '🗓 [SelectedDateNotifier] Moving to next day: ${state.toString()} → ${newDate.toString()}');
    state = newDate;
  }
...
final selectedDateProvider =
  ...
  // Add a listener to debug selected date changes using the non-deprecated method
  ref.listen<DateTime>(
    selectedDateProvider.select((state) => state),
    (previous, next) {
      if (previous != next) {
        ...
        debugPrint(
            '🗓 [selectedDateProvider] Day of week changed: $previousWeekday → $nextWeekday');
      }
    },
