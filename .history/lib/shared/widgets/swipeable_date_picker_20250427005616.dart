...
class _SwipeableDatePickerState extends ConsumerState<SwipeableDatePicker>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (currentDate.year != today.year ||
        ...
        debugPrint(
            '🗓 [SwipeableDatePicker] Initializing with today\'s date: ${today.toString()}');
        ref.read(selectedDateProvider.notifier).setDate(today);
      } else {
        debugPrint(
            '🗓 [SwipeableDatePicker] Current date already set to today: ${today.toString()}');
      }
    });
  }
  ...
  void _changeDate(int dayOffset) {
    ...
    debugPrint(
        '🗓 [SwipeableDatePicker] Triggering date change: $dayOffset days from $currentDate to $newDate');
    ...
    _pageController
      ...
      _isAnimating = false;
      debugPrint(
          '🗓 [SwipeableDatePicker] Animation complete, current date: ${ref.read(selectedDateProvider)}');
