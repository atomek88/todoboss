...
class DailyTodoCalendarHeatmap extends StatefulWidget {
  ...
  @override
  State<DailyTodoCalendarHeatmap> createState() =>
      _DailyTodoCalendarHeatmapState();
}

class _DailyTodoCalendarHeatmapState extends State<DailyTodoCalendarHeatmap> {
  // Initialize with empty lists to prevent late initialization errors
  List<Map<String, dynamic>> _weeks = [];
  List<DateTime> _monthTransitions = [];
  ...
  void _prepareData() {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 50), () {
      ...
      final startDate =
          endDate.subtract(Duration(days: widget.weeksToShow * 7 - 1));
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (widget.showDayLabels) _buildDayLabels(cellSize),
              ...
              Stack(
                children: [
                  ...
                  Column(
                    children: _weeks
                        .map((week) => _buildWeekRow(week, cellSize))
                        .toList(),
                  ),
                  ...
                  if (widget.showMonthLabels)
                    ..._monthTransitions.map((date) {
                      ...
                      final verticalOffset =
                          weekIndex * (cellSize + widget.cellSpacing);
  ...
  Widget _buildDayCell(Map<String, dynamic> day, double cellSize) {
    ...
    if (date != null && dailyTodo != null) {
      // Calculate the completion ratio for color intensity
      double completionRatio = 0.0;
      if (dailyTodo.taskGoal > 0) {
        completionRatio = dailyTodo.completedTodosCount / dailyTodo.taskGoal;
        completionRatio = completionRatio.clamp(0.0, 1.0); // Cap at 0.0-1.0
      }

      // Calculate the deletion ratio for color intensity
      double deletionRatio = 0.0;
      if (dailyTodo.todos.isNotEmpty) {
        deletionRatio = dailyTodo.deletedTodosCount / dailyTodo.todos.length;
        deletionRatio = deletionRatio.clamp(0.0, 1.0); // Cap at 0.0-1.0
      }

      // Apply completion color if there are completed todos
      if (dailyTodo.completedTodosCount > 0) {
        // Scale the opacity based on completion ratio
        final opacity = 0.3 + (completionRatio * 0.7); // Range: 0.3-1.0

        // Use gradient for high achievement (over 150% of goal)
        if (dailyTodo.taskGoal > 0 && 
            dailyTodo.completedTodosCount >= (dailyTodo.taskGoal * 1.5)) {
          useGradient = true;
          gradientColors = [widget.completedColor, Colors.teal];
        } else {
          cellColor = widget.completedColor.withOpacity(opacity);
        }
      }

      // Apply deletion color if it has a higher ratio or if there are no completed tasks
      if (dailyTodo.deletedTodosCount > 0 && 
          (deletionRatio > completionRatio || dailyTodo.completedTodosCount == 0)) {
        // Scale the opacity based on deletion ratio
        final opacity = 0.3 + (deletionRatio * 0.7); // Range: 0.3-1.0

        // Use gradient for many deletions (over 5)
        if (dailyTodo.deletedTodosCount > 5) {
          useGradient = true;
          gradientColors = [widget.deletedColor, Colors.orange];
        } else {
          cellColor = widget.deletedColor.withOpacity(opacity);
        }
      }

      // Add a star icon if the goal was met or exceeded
      if (dailyTodo.completedTodosCount >= dailyTodo.taskGoal && 
          dailyTodo.taskGoal > 0) {
        cellIcon = Icons.star;
      }
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        margin: EdgeInsets.all(widget.cellSpacing / 2),
        decoration: BoxDecoration(
          color: (date == null || useGradient) ? Colors.transparent : cellColor,
          gradient: (date != null && useGradient)
              ? LinearGradient(
          ...
          boxShadow: (date != null && (useGradient || cellIcon != null))
              ? [
                  BoxShadow(
        ...
        child: cellIcon != null
            ...
            : dailyTodo != null && dailyTodo.deletedTodosCount > 0
                ? Center(
                    child: Container(
                      width: cellSize * 0.35,
  ...
  /// Group dates by week
  List<Map<String, dynamic>> _groupByWeek(DateTime startDate, DateTime endDate,
