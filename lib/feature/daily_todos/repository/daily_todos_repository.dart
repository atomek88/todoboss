...
import 'package:todoApp/feature/daily_todos/models/daily_todo_isar.dart';
...
class DailyTodoRepository {
  ...
  Future<List<DailyTodo>> getPastDailyTodos() async {
    try {
      ...
      return allDailyTodos
          .where((dailyTodo) => dailyTodo.date.isBefore(normalizedToday))
          .toList()
  ...
  Future<DailyTodo?> setTaskGoal(DateTime date, int goal) async {
    try {
      ...
      final dailyTodo = await getDailyTodoForDate(date);
      final updatedDailyTodo = dailyTodo.copyWith(taskGoal: goal);

      await saveDailyTodo(updatedDailyTodo);

      debugPrint(
          '📅 [DailyTodoRepository] Set task goal for ${dailyTodo.id} to $goal');
      return updatedDailyTodo;
    } catch (e) {
